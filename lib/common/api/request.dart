...
class RequestModel {
  ...
  const RequestModel({this.parameters, this.headers, this.data});
  ...
  const RequestModel.create({
    ...
    dynamic data,
  }) : this(parameters: parameters, headers: headers, data: data);

  // 构造为查询格式
  const RequestModel.query({
    required Map<String, dynamic>? parameters,
    Map<String, dynamic>? headers,
  }) : this(parameters: parameters, headers: headers, data: null);
  ...
  static RequestFormBuilder form({
    ...
    Map<String, dynamic>? data,
      ...
      RequestFormBuilder(parameters: parameters, headers: headers, data: data);
}
...
class RequestFormBuilder extends RequestModel {
  RequestFormBuilder({
    ...
    super.headers,
    Map<String, dynamic>? data,
  }) : super(data: FormData.fromMap(data ?? {}));
  ...
  // 添加多个文件
  RequestFormBuilder addFilesSync(String key, List<RequestFileItem> files) =>
      this
        ..data.files.addAll(
          files.map((item) {
            final value = MultipartFile.fromFileSync(
              item.filePath,
...
class RequestFileItem {
  ...
  RequestFileItem({required this.filePath, this.filename, this.mediaType});
}
